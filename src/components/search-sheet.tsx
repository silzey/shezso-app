...
export function SearchSheet({ open, onOpenChange }: SearchSheetProps) {
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent side="bottom" className="h-[90%] flex flex-col p-0 bg-background/80 backdrop-blur-lg border-0 shadow-none">
                <SheetHeader className="p-4 border-b flex-shrink-0">
                    <SheetTitle className="flex items-center text-primary"><Search className="mr-2 h-5 w-5" />Search Products & Stores</SheetTitle>
                </SheetHeader>
                ...
                <SheetFooter className="p-4 border-t bg-transparent">
                     <div className="relative w-full">
                        ...
                        <Input
                            ...
                            className="pl-10 bg-muted/50 text-foreground placeholder:text-muted-foreground"
                            value={searchTerm}
...
const SearchResultCard = ({ result, onLinkClick }: { result: SearchResult, onLinkClick: () => void }) => {
    if (result.type === 'product') {
        ...
        return (
            <Card className="overflow-hidden shadow-sm bg-card/60 backdrop-blur-sm">
                <CardContent className="p-3 flex items-center gap-4">
                    ...
                    <div className="flex-grow min-w-0">
                        <p className="text-xs text-primary font-medium flex items-center gap-1"><Package className="h-3 w-3" />PRODUCT</p>
                        <h4 className="font-semibold text-sm truncate text-foreground" title={product.name}>{product.name}</h4>
        ...
        return (
            <Card className="overflow-hidden shadow-sm bg-card/60 backdrop-blur-sm">
                <CardContent className="p-3 flex items-center gap-4">
                    ...
                    <div className="flex-grow min-w-0">
                        <p className="text-xs text-primary font-medium flex items-center gap-1"><Store className="h-3 w-3" />DISPENSARY</p>
                        <h4 className="font-semibold text-sm truncate text-foreground" title={dispensary.name}>{dispensary.name}</h4>
